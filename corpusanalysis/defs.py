...
def var_augmented(self, var, decoration=True):
...
def vars_augmented(self,vars, decoration=True):
    return [var_augmented(self,v,decoration) for v in vars]


def nom_augmented(self, nom,decoration=True):
    ...
    try:
        ...
        if decoration:
            if id:
                thm = "<sup ><small><a title='Open in Thamous' href='https://thamous.univ-rennes1.fr/php/form_ref.php?id_ref=" + str(
                    id) + "&table=t" + table + "&projet=" + prjt + "' target='_blank' style='color:black; text-decoration: none'>Thm</a>" \
                                                                   "  <a title='Thamous navigation' href='https://thamous.univ-rennes1.fr/php/navigation_liens_invariante.php?id_origine=" + str(
                    id) + "&table_origine=t" + table + "&projet=" + prjt + "&type_liens=Tous' target='_blank' style='color:black; text-decoration: none'>&harr;</a></small></sup>"
                nomAug += thm
