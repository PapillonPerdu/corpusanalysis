...
def add_vars(self, newVars, var = '', position = 'after'):
    ...
    varsAdded = []
    for newVar in reversed(newVars):
        if newVar in self.vars:
            print(color.bold + '''The variable "{}" already exists.'''.format(newVar) + color.end)
        else:
            add_var_db(self, newVar, var = var, position = position)
...
def add_name(self, newNom, name ='', position ='after', table='', id='', project=''):
    ...
    if self.source == 'db':
        add_nom_db(self, newNom, nom = name, position = position)
        # updates arrays
        load_db(self, self.db)
        if table and id:
            thm_def = thamous_ref(id, table)  # definition of name from Thamous database
            write_name_definition(self, newNom, thm_def)
            write_name_thamous(self, newNom, id, table, project)
    else:
...
def add_noms(self, newNoms, name='', position='after'):
    if self.source != 'db':
    ...
    nomsAdded = []
    for newNom in reversed(newNoms):
        if newNom in self.noms:
            print(color.bold + '''The name "{}" already exists.'''.format(newNom) + color.end)
        else:
            add_nom_db(self, newNom, nom=name, position=position)
    ...
    print('''The names "{}" have been added.'''.format(', '.join(nomsAdded)))


def add_names_type(self, newT, namesType  ='', position ='after'):
    namesType = html.unescape(namesType)
    if newT in self.noms_types_types:
        print(color.bold + '''The type "{}" already exists.'''.format(newT) + color.end)
        sys.exit()

    if self.source == 'db':
        add_noms_type_db(self, newT, nomsType = namesType, position = position)
    else:
...
def replace_value(self, value, newValue,indexesNames, indexesVars):
    for n in indexesNames:
        for v in indexesVars :
            if self.data[n][v]==value : write_val(self,self.noms[n],self.vars[v],newValue)
...
def write_name_definition(self, nom, definition):
    nom = html.unescape(nom)
    definition = html.unescape(definition)
    if self.source == 'csv':
...
def write_name_thamous(self, nom, id, table, projet):
    """Record the table in Thamous, the id, and the projet of name."""
    if self.source == 'csv':
...
def write_vars(self, indexNom,indexesVars, value=''):
    initValue = value
    for v in indexesVars:
        if not initValue:
            ...
            value = initValue
        if value != self.data[indexNom][v] :
            write_val(self, self.noms[indexNom], self.vars[v], value)


def write_names(self, indexesNoms, indexesVars):
...
def write_names_value(self, value, indexVar,indexesNoms):
    for n in indexesNoms:
        write_val(self, self.noms[n], self.vars[indexVar], value)
...
def write_type_vars(self, varsType, vars=[], varSauf=[],
    ...
    indexesVars = indexesNomsVars['vars']
    for v in indexesVars:
...
def write_nom_types(self, nom, nomsTypes=[], nomsTypeSauf=[]):
    ...
    for type in types:
        ...
        if value :
            write_name_type_val(self, type, nom, value)

def write_type_names(self, nomsType, noms=[], nomSauf=[],
                     nomsTypes=[], nomsTypeSauf=[], nomsTypesFormule='', corpus = 'all'):

    if not nomsType in self.noms_types_types:
        print('The name type "{}" doesn\'t exist.'.format(nomsType))
        sys.exit()
    ...
    for n in indexesNoms:
        ...
        if value :
            write_name_type_val(self, type, self.noms[n], value)


def copy_vars(self, indexNom1,indexNom2,indexesVars):
    """Copy the values of indexNoms to indexNom2"""
    for v in indexesVars:
        write_val(self, self.noms[indexNom2], self.vars[v], self.data[indexNom1][v])
