...
def getIndexesVarsTypes(self,
    ...
    #TODO : doublon avec varsTypesToIndexesTypes
...
def show_name_types(self, indexNom,indexesNomsTypeSortie,pasColonne=10):
    indexesTypes = [t for t in indexesNomsTypeSortie if self.noms_types_data[indexNom][t]=='*']
    columns = [self.names_types_types[t] for t in indexesTypes]
    lines = [[self.names_types_data_augmented[indexNom][t] for t in indexesTypes]]

    printLines(lines, columns=columns, index=[self.names[indexNom]],pasColonne=pasColonne)

def show_var_types(self, indexVar,indexesVarsTypeSortie,pasLigne=10):
    indexesTypes = [t for t in indexesVarsTypeSortie if self.vars_types_data[t][indexVar]=='*']
    index = [self.vars_types_types[t] for t in indexesTypes]
    lines = [[self.vars_types_data_augmented[t][indexVar]] for t in indexesTypes]

    printLines(lines, columns=[self.vars[indexVar]], index=index,pasLigne=pasLigne)
...
def show_vars_types_selected(self, indexesVars,indexesVarsTypeSortie,
                    pasColonne= 10,pasLigne=10):
    indexesTypes = [t for t in indexesVarsTypeSortie if not len([v for v in indexesVars if self.vars_types_data[t][v]== '*'])==0]
    index = [self.vars_types_types[t] for t in indexesTypes]
    columns = [var_augmented(self, self.vars[v]) for v in indexesVars if self.vars[v] in self.vars_types_vars]
    lines = [[self.vars_types_data_augmented[t][v] for v in indexesVars] for t in indexesTypes]

    print('variables : ' + str(len(indexesVars)))
    print('types : ' + str(len(indexesTypes)))
    print(color.bold + 'Variables types : ' + color.end)
    printLines(lines, columns=columns, index=index,pasColonne=pasColonne,pasLigne=pasLigne)

def show_names_types(self, indexesNoms, indexesNomsTypeSortie, pasColonne=10, pasLigne=10):
