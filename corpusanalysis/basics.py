...
def show_dashboard(self):
    ...
    display(HTML(df.to_html(escape=False)))
def equalExtSub(val1,val2, indexName1, indexName2, indexVar, equalize) :
    for equalizer in equalize :
        b1 = [val1,val2] in equalizer['equals'] or [val2,val1] in equalizer['equals']
        b2 = indexName1 in equalizer['indexes']['names'] and indexName2 in equalizer['indexes']['names'] \
             and indexVar in equalizer['indexes']['vars']
        if (b1 and b2) :
            return True
    return False

def equalExt(self, indexName1,indexName2,indexVar, equalize) :
    val1 = self.data[indexName1][indexVar]
    val2 = self.data[indexName2][indexVar]
    return val1 == val2 or \
        [val1,val2, indexName1,indexVar] in equalize or [val1,val2, indexName2,indexVar] or \
    [val2, val1, indexName1, indexVar] in equalize or [val2, val1, indexName2, indexVar]
...
def show_data(self,indexesNoms,indexesVars,variantes=[],
    ...
    for i in indexesNoms :
        if values :
            if decoration :
                lines.append([reduceVariante(self.data_augmented[i][k],variantes) for k in indexesVars])
            elif replaceValues:
                lines.append(
                    [reduceVariante(self.data_augmented[i][k],variantes) if reduceVariante(self.data[i][k],variantes) not in valuesReplaced else
                     replaceValue(reduceVariante(self.data[i][k],variantes),k) for k in
                     indexesVars])
            else :
                lines.append([reduceVariante(self.data_augmented[i][k],variantes) for k in indexesVars])
...
def indexes_like(self,
                 indexNom, indexesNoms, indexesVars, pourcent, variantes=[]):
    """
    ...
    indexesNomsRes = [i for i in indexesNoms if \
                      listsEqual(self.data[indexNom], self.data[i], indexesVars,variantes=variantes, precision= pourcent)]
...
def like(self,indexNom,indexesNoms,indexesVars,
    ...
    indexesNomsRes = indexes_like(self,indexNom, indexesNoms, indexesVars, pourcent, variantes=variantes)
...
def communsStrict(self,
                  indexNom1, indexNom2, indexesVars, variantes=[]):
    """liste des valeurs définies de premier nom, contenues dans celles de deuxième nom, parmi des variables, donnés par leurs indexes. """
    comp = []
...
def vars_difference(self ,indexNom1 ,indexNom2 ,indexesVars, variantes=[]):

    diff = differenceStrict(self ,indexNom1, indexNom2, indexesVars, variantes)
...
def show_difference(self,
    ...
    diff = [differenceStrict(self ,indexNom1, indexNom2, indexesVars,variantes),
            difference(self ,indexNom2, indexNom1, indexesVars,variantes)]
...
def show_difference_types(self,
                          indexNom1, indexNom2, indexesVars,
                          indexesVarsTypeSortie,variantes=[],
...
def vars_common(self, indexNom1, indexNom2, indexesVars,variantes=[]):
    com = communsStrict(self ,indexNom1, indexNom2, indexesVars, variantes=variantes)
...
def show_common(self, indexNom1, indexNom2, indexesVars, variantes=[]):
    com = [communsStrict(self ,indexNom1, indexNom2, indexesVars, variantes)]
...
def show_names_common_percent(self,
    ...
    totaux = []
    for v in indexesVars:
        indexesNomsCommun = indexes_like(self ,indexNom, indexesNoms, [v], 100, variantes=variantes)
...
def prox(L1, L2, poids, variantes=[]):
    prox = 0
...
def vars_relative_difference(self, indexNom, indexesNoms, indexesVars, variantes=[]):
...
def show_difference_relative(self,indexNom,indexesNoms,indexesVars, variantes=[]):
    varsDiff = self.vars_relative_difference(indexNom, indexesNoms, indexesVars,variantes)
...
def vars_relative_sum(self, indexNom, indexesNoms, indexesVars, variantes=[]):
...
def equalVals(self, indexNom, indexesVars, values,variantes=[]):
    '''
...
def show_vars_type(self, tp, decoration=True):
    varsT = vars_augmented(self,typeToVars(self,tp, decoration))
...
def indexesVarsToVals(self, indexesNoms, indexesVars, variantes=[]):
    values = [reduceVariante(str(self.data[n][v]), variantes) for v in indexesVars for n in indexesNoms]
    return sorted(values)
...
def typeToVals(self, indexNom, tp, variantes=[]):
    indexesVarsType = typeToIndexesVars(self,tp, variantes=variantes)
    values = [reduceVariantes(str(self.data[indexNom][v]),variantes) for v in indexesVarsType]
    return sorted(values)
