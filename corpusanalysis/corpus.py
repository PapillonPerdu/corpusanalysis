...
def show_vars_missing(self, indexNom, indexesVars):
    """
...
def show_vars_without_types(self, indexesVars):
...
def show_names_without_types(self, indexesNoms):
...
def show_names_without_value(self, indexesNoms, indexesVars):
    """
...
def names_thamous_missing(self, indexesNoms):
...
def show_values_without_quotations(self, indexesNoms, indexesVars):
    """
...
#vars non définies d'une édition
def vars_without_value(self, indexNom, indexesVars):
    resVars=[self.vars[v] for v in indexesVars if self.data[indexNom][v] in self.nuls ]
    return resVars


#vars définies d'une édition
def vars_with_value(self, indexNom, indexesVars):
    resVars=[self.vars[v] for v in indexesVars if not self.data[indexNom][v] in self.nuls ]
    return resVars

#vars non définies pour une liste de noms
def vars_without_values(self, indexesNoms, indexesVars):
    resVars=[]
    for v in indexesVars:
        allNull=True
        for n in indexesNoms:
            if not self.data[n][v] in self.nuls:
                allNull=False
                break
        if allNull : resVars.append(self.vars[v])
    return resVars
