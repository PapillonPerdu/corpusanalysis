...
def getIndexes(self, noms, nomSauf, nomsTypes, nomsTypeSauf, nomsTypesFormule,
    ...
    if not (corpus == 'all' and domaine == 'all'):
        indexes = corpusdomaine(self, corpus, domaine, indexesNoms, indexesVars)
    else:
...
def show_dashboard(self):
    ...
    if self.nomsDefs_exists:
        ldn = len(self.noms) - len(self.noms_sans_def)
        lndn = len(self.noms_sans_def)
    else:
        ldn = 0
        lndn = len(self.noms)
    ...
    if self.nomsTypes_exists :
        ...
        ldnt = len(self.noms_types_avec_def)
        lndnt = len(self.noms_types_sans_def)
    else:
        lntt=0
        ldnt='-'
        lndnt='-'
    ...
    if self.varsTypes_exists :
        ...
        ldvt = len(self.vars_types_avec_def)
        lndvt = len(self.vars_types_sans_def)
    else:
        lvtt=0
        ldvt='-'
        lndvt='-'
...
def show_data(self,indexesNoms,indexesVars,
    ...
    if (len(indexesVars) == len(self.vars) or len(indexesNoms) == len(self.noms)) and len(indexesVars) >= 50 and len(indexesNoms) >= 10:
        answer =  input("Show all the variables ? It can be quite long...")
    ...
    printLines(lines, columns=resVars, index=resNoms,  pasColonne=pasColonne, pasLigne=pasLigne,width=width)


def view_data(self,indexesNoms,indexesVars,
              pasColonne, pasLigne,
              values = True, citations = False, width = ''):

    df = data_frame(self, indexesNoms,indexesVars,
...
def show_noms(self,indexesNoms):
    """
...
def interNoms(self,
    ...
    if nom2 == '': nom2 = nom1
    try:
        index1 = self.noms.index(nom1)
    except:
        print("L'édition \"" + nom1 + "\" n'existe pas.")
    try:
        index2 = self.noms.index(nom2) + 1
    except:
...
def show_like(self,
    ...
    resNomsLignes = [
        [self.noms[i]] + stats(self,self.communs(indexNom, i, indexesVars), indexesVars) + [self.data[i][k] for
    ...
    printLines(resLignes, columns=resVars, index=resNoms)
...
def vars_difference_relative(self,indexNom, indexesNoms,indexesVars):
...
def show_difference_relative(self,indexNom,indexesNoms,indexesVars):
    varsDiff = self.vars_difference_relative(indexNom,indexesNoms,indexesVars)
...
def vars_somme_relative(self, indexNom,indexesNoms, indexesVars):
...
def indexesVarsDefiniesTypeNom(self,
    ...
    trueVars = [i for i in indexesVarsT if not self.data[indexNom][i] in self.exclus]
    return trueVars
...
def totalVarsDefiniesTypeNom(self,
                             indexNom, indexesVars, tp):
    trueVars = indexesVarsDefiniesTypeNom(self,indexNom, indexesVars, tp)
    return len(trueVars)
...
def indexesVarsDefiniesType(self,
    ...
    trueVars = []
    for n in indexesNoms:
        trueVars += indexesVarsDefiniesTypeNom(self,n, indexesVars, tp)
...
def totalVarsDefiniesType(self,
                          indexesNoms, indexesVars, tp):
    trueVars = indexesVarsDefiniesType(self,indexesNoms, indexesVars, tp)
    return len(trueVars)
...
#                  tp, indexesVars):
# indexesVarsType = [v for v in indexesVars if v in typeToIndexesVars(self,tp)]
...
def show_manque(self, indexNom,indexesVars):
...
def noms_defs(self, str, indexesNoms):
