...
def includes(l, M):
    ''' test if list l contains a list of  M'''
    for m in M :
...
def inList(l, M):
    ''' test if list l is contained in a list of  M'''
    for m in M:
...
#Delete from str the characters from the list variantes
#variantes=["a","a"]] : replace "a'" by "a"
def reduceVariante(str,variantes) :
    variantes=map(re.escape,variantes)
    pat='|'.join(variantes)
...
# avec une liste donnée
def sumEqualPourcentTest(ListeL, L, indexesVars, pourcent,variantes=[]):
    sum = 0
    for i in indexesVars:
        ok = False
        for l in ListeL:
