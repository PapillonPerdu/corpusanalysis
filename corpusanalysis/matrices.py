...
def show_distance_matrice_coordonnees(self,indexesNoms,indexesVars):
...
def show_distance_ordonnee(self, indexesNoms,indexesVars,
    ...
    distEtNoms = [[self.distance_matrice[i][j],
                  ...
                  in np.ndindex(self.distance_matrice.shape) if i < j and pourcent <= int(
            round(self.distance_matrice[i][j] * 100 / self.selectedDistMax)) <= Pourcent]
...
def show_graphe_distance(self,indexesNoms,indexesVars, liens=True,
    ...
    graphe_matrice_distance(self,liens, indexesNoms, indexesVars, pourcent, Pourcent, couleursLiens)
    plt.show()
...
def show_proximite_matrice_coordonnees(self,indexesNoms,indexesVars):
    updateProxMatrices(self,indexesVars, indexesNoms)
...
def show_proximite_ordonnee(self, noms=[], nomSauf=[],
    ...
    proxEtNoms = [[self.proximite_matrice[i][j],
                   nom_augmented(self,redindexNomToNom(self,j, indexesNoms)) + ' - ' + nom_augmented(self,
                       redindexNomToNom(self,i, indexesNoms))] for i, j
                  in np.ndindex(self.proximite_matrice.shape) if i < j and Pourcent >= int(
            round(self.proximite_matrice[i][j] * 100 / self.selectedDistMax)) >= pourcent]
...
def show_graphe_proximite(self,indexesNoms,indexesVars, liens=True,
                          pourcent=0, Pourcent=100, couleursLiens={}):
